 ...
 * business change it describes — V4__outbox_events.sql, ADR-006.
 *
 ...
 * transaction committed rather than rebuilding one at send time (which would re-stamp
 * {@code occurredAt} and lose the original {@code correlationId}). The column is {@code jsonb} per
 * the frozen schema, so the stored <em>text</em> is PostgreSQL's normalization of that document —
 * see {@code OutboxDispatcher#wireForm}.
 */
@Entity
