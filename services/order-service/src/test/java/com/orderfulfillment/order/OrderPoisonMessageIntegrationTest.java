...
/**
 * Scenario 6 — Poison Message / DLQ (docs/scenarios.md), Order Service's half.
 *
 * <p>Both poison records here are the kinds the scenario names — "an unparseable {@code payload}
 * body, or an {@code eventVersion} no consumer knows" — and both are put on
 * {@code inventory.events} as real bytes, so the real {@link OrderInventoryEventsConsumer} and the
 * real error handler decide what happens. Nothing is stubbed.
 *
 ...
 * classification is real — a service that retried these anyway would fail this test.
 *
 ...
 */
class OrderPoisonMessageIntegrationTest extends AbstractIntegrationTest {
    ...
    void anEnvelopeWithAnUnknownEventVersionIsDeadLetteredWithoutRetrying() {
        ...
        String poison = """
                {"eventId":"%s","eventType":"%s","eventVersion":99,"occurredAt":"%s",
                 ...
                 "payload":{"orderId":"%s","reservationId":"resv-poison",
                            "items":[{"sku":"SKU-001","quantity":1}],"reservedAt":"%s"}}
                """.formatted(UUID.randomUUID(), EventTypes.INVENTORY_RESERVED, Instant.now(),
                UUID.randomUUID(), orderId, orderId, Instant.now());
    ...
    void anUnparseablePayloadIsDeadLetteredWithoutRetrying() {
        ...
        // string where an InventoryReserved object belongs. It gets past decode() and fails in
        ...
        String poison = """
                {"eventId":"%s","eventType":"%s","eventVersion":%d,"occurredAt":"%s",
                 "correlationId":"%s","aggregateId":"%s","payload":"this is not an InventoryReserved"}
                """.formatted(UUID.randomUUID(), EventTypes.INVENTORY_RESERVED, EventTypes.CURRENT_VERSION,
    ...
    private void assertCommonMetadata(ConsumerRecord<String, String> dlq, String orderId) {
        ...
        assertThat(header(dlq, KafkaHeaders.DLT_ORIGINAL_TOPIC)).isEqualTo(KafkaTopics.INVENTORY_EVENTS);
        ...
        assertThat(header(dlq, KafkaHeaders.DLT_ORIGINAL_CONSUMER_GROUP)).isEqualTo("order-service");
    ...
    private ConsumerRecord<String, String> publishAndAwaitDlq(String orderId, String poison) {
        Consumer<String, String> dlqConsumer = rawConsumer(KafkaTopics.ORDERS_DLQ);
        try {
            kafkaTemplate.send(KafkaTopics.INVENTORY_EVENTS, orderId, poison);
            ...
            assertThat(matches).as("poison record for %s should land on %s", orderId, KafkaTopics.ORDERS_DLQ)
                    .hasSize(1);
