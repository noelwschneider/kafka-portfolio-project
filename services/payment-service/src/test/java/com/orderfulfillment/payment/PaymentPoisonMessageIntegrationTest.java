...
/**
 * Scenario 6 — Poison Message / DLQ (docs/scenarios.md), Payment Service's half.
 *
 ...
 * as real bytes, so the real {@link PaymentOrderEventsConsumer} and the real error handler decide
 ...
 */
class PaymentPoisonMessageIntegrationTest extends AbstractIntegrationTest {
    ...
    void anEnvelopeWithAnUnknownEventVersionIsDeadLetteredWithoutRetrying() {
        ...
        String poison = """
                {"eventId":"%s","eventType":"%s","eventVersion":99,"occurredAt":"%s",
                 ...
                 "payload":{"orderId":"%s","amount":42.00,"idempotencyKey":"%s"}}
                """.formatted(UUID.randomUUID(), EventTypes.PAYMENT_REQUESTED, Instant.now(),
                UUID.randomUUID(), orderId, orderId, UUID.randomUUID());
    ...
    void anUnparseablePayloadIsDeadLetteredWithoutRetrying() {
        ...
        // string where a PaymentRequested object belongs. It gets past decode() and fails in
        ...
        String poison = """
                {"eventId":"%s","eventType":"%s","eventVersion":%d,"occurredAt":"%s",
                 "correlationId":"%s","aggregateId":"%s","payload":"this is not a PaymentRequested"}
                """.formatted(UUID.randomUUID(), EventTypes.PAYMENT_REQUESTED, EventTypes.CURRENT_VERSION,
    ...
    private void assertCommonMetadata(ConsumerRecord<String, String> dlq, String orderId) {
        ...
        assertThat(header(dlq, KafkaHeaders.DLT_ORIGINAL_CONSUMER_GROUP)).isEqualTo("payment-service");
    ...
    private ConsumerRecord<String, String> publishAndAwaitDlq(String orderId, String poison) {
        Consumer<String, String> dlqConsumer = rawConsumer(KafkaTopics.PAYMENTS_DLQ);
        try {
            ...
            assertThat(matches).as("poison record for %s should land on %s", orderId, KafkaTopics.PAYMENTS_DLQ)
                    .hasSize(1);
