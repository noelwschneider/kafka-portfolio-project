...
/**
 * Payment Service's entire share of the retry/DLQ policy: name the DLQ topic, and let the shared
 ...
 * listener container factory, so this one bean covers Payment Service's one listener without it
 * having to name it.
 */
@Configuration
public class PaymentKafkaReliabilityConfig {

    @Bean
    public DefaultErrorHandler kafkaErrorHandler(ConsumerErrorHandlerFactory factory) {
        return factory.create(KafkaTopics.PAYMENTS_DLQ);
    }
}
