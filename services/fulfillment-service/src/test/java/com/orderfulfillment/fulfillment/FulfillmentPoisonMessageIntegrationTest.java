...
/**
 * Scenario 6 — Poison Message / DLQ (docs/scenarios.md), Fulfillment Service's half.
 *
 * <p>Both poison records here are the kinds the scenario names — "an unparseable {@code payload}
 * body, or an {@code eventVersion} no consumer knows" — and both are put on {@code payments.events}
 * as real bytes, so the real {@link FulfillmentPaymentEventsConsumer} and the real error handler
 * decide what happens. Nothing is stubbed.
 *
 ...
 */
class FulfillmentPoisonMessageIntegrationTest extends AbstractIntegrationTest {
    ...
    void anEnvelopeWithAnUnknownEventVersionIsDeadLetteredWithoutRetrying() {
        ...
        String poison = """
                {"eventId":"%s","eventType":"%s","eventVersion":99,"occurredAt":"%s",
                 ...
                 "payload":{"orderId":"%s","paymentAttemptId":"pay-poison-1","amount":10.00,
                            "authorizedAt":"%s"}}
                """.formatted(UUID.randomUUID(), EventTypes.PAYMENT_AUTHORIZED, Instant.now(),
    ...
    void anUnparseablePayloadIsDeadLetteredWithoutRetrying() {
        ...
        // string where a PaymentAuthorized object belongs. It gets past decode() and fails in
    ...
    private void assertCommonMetadata(ConsumerRecord<String, String> dlq, String orderId) {
        ...
        assertThat(header(dlq, KafkaHeaders.DLT_ORIGINAL_TOPIC)).isEqualTo(KafkaTopics.PAYMENTS_EVENTS);
        ...
        assertThat(header(dlq, KafkaHeaders.DLT_ORIGINAL_CONSUMER_GROUP)).isEqualTo("fulfillment-service");
    ...
    private ConsumerRecord<String, String> publishAndAwaitDlq(String orderId, String poison) {
        Consumer<String, String> dlqConsumer = rawConsumer(KafkaTopics.FULFILLMENT_DLQ);
        try {
            ...
            assertThat(matches).as("poison record for %s should land on %s", orderId, KafkaTopics.FULFILLMENT_DLQ)
                    .hasSize(1);
