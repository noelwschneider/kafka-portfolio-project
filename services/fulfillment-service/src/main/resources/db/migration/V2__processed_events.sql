-- Fulfillment Service's copy of the idempotency ledger. The shape is frozen in docs/db-ownership.md
...
-- consumers: PaymentAuthorized is consumed independently by both Order Service and Fulfillment
-- Service (docs/events/event-catalog.md §3), and each keeps its own ledger row for it.
