...
/**
 * The read half of ADR-006's transactional outbox for Fulfillment Service: one batch of pending
 * rows, sent to {@code fulfillment.events} in insertion order and marked published. Same shape as
 * Order Service's {@code OutboxDispatcher} — see that class's Javadoc for the full rationale.
 */
@Component
    ...
    int publishPendingBatch() {
        ...
        for (OutboxEventEntity row : pending) {
            try {
                ...
                kafkaTemplate.send(KafkaTopics.FULFILLMENT_EVENTS, row.getAggregateId(), wireForm(envelopeNode))
