...
/**
 * The write half of ADR-006's transactional outbox, extended to Fulfillment Service in Sprint 2.
 * Same shape as Order Service's {@code OutboxRecorder} — see that class's Javadoc. {@link
 * Propagation#MANDATORY} because an outbox insert in its own transaction would reintroduce the
 * dual-write window this class exists to close; the call site is always {@link FulfillmentService
 * #createShipment}'s {@code REQUIRES_NEW} transaction, the same one that claims the
