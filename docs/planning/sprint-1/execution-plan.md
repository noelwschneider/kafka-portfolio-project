...
| Scenario Service (new backend service) | 5 | Sonnet | Medium | Missing from this table until Phase 5 caught it — REST + persistence + event publish/consume against the already-frozen `scenario-service.yaml`, by analogy to the Order Service and Payment+Fulfillment rows above. Drives the other four services' real APIs and owns the event-projection table backing the Event Explorer; not smaller in scope than those rows despite the gap. See `docs/agent-reports/phase-5-scenario-service.md` §7. |
